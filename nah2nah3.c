...
static void draw_title_menu(Canvas* canvas, GameContext* ctx) {
    ...
    if(ctx->selected_side == 0) {
        if(row == 0) { // Zero Hero
            ...
            canvas_draw_str(canvas, 16, y_offset + 28, frame == 0 ? "(o_|o)!" : " !(0 |o)"); // head?
...
static void input_callback(InputEvent* input, void* ctx_ptr) {
    ...
    if(ctx->state == GAME_STATE_LOADING) {
        ...
        if(is_press) {
            ...
            ctx->state = ctx->selected_game == GAME_MODE_ZERO_HERO ? GAME_STATE_ZERO_HERO : ctx->selected_game == GAME_MODE_FLIP_ZIP ? GAME_STATE_FLIP_ZIP : GAME_STATE_ZERO_HERO; // Placeholder for new games
        ...
        if(is_short && input->key == InputKeyOk) {
            ctx->state = ctx->selected_game == GAME_MODE_ZERO_HERO ? GAME_STATE_ZERO_HERO : ctx->selected_game == GAME_MODE_FLIP_ZIP ? GAME_STATE_FLIP_ZIP : GAME_STATE_ZERO_HERO;
            ctx->pause_back_count = 0;
